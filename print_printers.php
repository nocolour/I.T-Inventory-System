...
$total_stmt = $pdo->query("SELECT COUNT(*) FROM printers");
$total_rows = $total_stmt->fetchColumn();
$total_pages = ceil($total_rows / $limit);

// Fetch paginated printers from the database
$stmt = $pdo->prepare("SELECT * FROM printers ORDER BY device_name ASC LIMIT :start, :limit");
...
<html>
<head>
    <title>Printers Inventory</title>
    ...
    <table id="printersTable">
        ...
        <tbody>
            <?php foreach ($printers as $printer): ?>
                <tr>
                    <td><?= htmlspecialchars($printer['device_name']) ?></td>
                    <td><?= htmlspecialchars($printer['brand']) ?></td>
                    <td><?= htmlspecialchars($printer['model']) ?></td>
                    <td><?= htmlspecialchars($printer['serial_number']) ?></td>
                    <td><?= htmlspecialchars($printer['processor']) ?></td>
                    <td><?= htmlspecialchars($printer['ram']) ?></td>
                    <td><?= htmlspecialchars($printer['storage']) ?></td>
                    <td><?= htmlspecialchars($printer['ip_address']) ?></td>
                    <td><?= htmlspecialchars($printer['mac_address']) ?></td>
                    <td><?= htmlspecialchars($printer['location']) ?></td>
                    <td><?= htmlspecialchars($printer['existing_user']) ?></td>
                    <td><?= htmlspecialchars($printer['status']) ?></td>
                    <td><?= htmlspecialchars($printer['purchase_date']) ?></td>
                    <td><?= htmlspecialchars($printer['warranty']) ?></td>
                    <td><?= htmlspecialchars($printer['other_details']) ?></td>
                </tr>
            <?php endforeach; ?>
        </tbody>
    </table>
