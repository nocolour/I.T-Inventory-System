...
$total_stmt = $pdo->query("SELECT COUNT(*) FROM servers");
$total_rows = $total_stmt->fetchColumn();
$total_pages = ceil($total_rows / $limit);

// Fetch paginated servers from the database
$stmt = $pdo->prepare("SELECT * FROM servers ORDER BY device_name ASC LIMIT :start, :limit");
    ...
    <table id="serversTable">
        ...
        <tbody>
            <?php foreach ($servers as $server): ?>
                <tr>
                    <td><?= htmlspecialchars($server['device_name']) ?></td>
                    <td><?= htmlspecialchars($server['brand']) ?></td>
                    <td><?= htmlspecialchars($server['model']) ?></td>
                    <td><?= htmlspecialchars($server['serial_number']) ?></td>
                    <td><?= htmlspecialchars($server['processor']) ?></td>
                    <td><?= htmlspecialchars($server['ram']) ?></td>
                    <td><?= htmlspecialchars($server['storage']) ?></td>
                    <td><?= htmlspecialchars($server['ip_address']) ?></td>
                    <td><?= htmlspecialchars($server['mac_address']) ?></td>
                    <td><?= htmlspecialchars($server['location']) ?></td>
                    <td><?= htmlspecialchars($server['existing_user']) ?></td>
                    <td><?= htmlspecialchars($server['status']) ?></td>
                    <td><?= htmlspecialchars($server['purchase_date']) ?></td>
                    <td><?= htmlspecialchars($server['warranty']) ?></td>
                    <td><?= htmlspecialchars($server['other_details']) ?></td>
                </tr>
            <?php endforeach; ?>
        </tbody>
    </table>
