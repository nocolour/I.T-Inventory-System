...
$total_stmt = $pdo->query("SELECT COUNT(*) FROM tablets");
$total_rows = $total_stmt->fetchColumn();
$total_pages = ceil($total_rows / $limit);

// Fetch paginated tablets from the database
$stmt = $pdo->prepare("SELECT * FROM tablets ORDER BY device_name ASC LIMIT :start, :limit");
...
<head>
    ...
    <script>
        ...
        function toggleColumn(columnIndex) {
            const table = document.getElementById('tabletsTable');
        ...
        function searchTable() {
            ...
            const rows = document.querySelectorAll('#tabletsTable tbody tr');
            rows.forEach(row => {
    ...
    <table id="tabletsTable">
        ...
        <tbody>
            <?php foreach ($tablets as $tablet): ?>
                <tr>
                    <td><?= htmlspecialchars($tablet['device_name']) ?></td>
                    <td><?= htmlspecialchars($tablet['brand']) ?></td>
                    <td><?= htmlspecialchars($tablet['model']) ?></td>
                    <td><?= htmlspecialchars($tablet['serial_number']) ?></td>
                    <td><?= htmlspecialchars($tablet['processor']) ?></td>
                    <td><?= htmlspecialchars($tablet['ram']) ?></td>
                    <td><?= htmlspecialchars($tablet['storage']) ?></td>
                    <td><?= htmlspecialchars($tablet['ip_address']) ?></td>
                    <td><?= htmlspecialchars($tablet['mac_address']) ?></td>
                    <td><?= htmlspecialchars($tablet['location']) ?></td>
                    <td><?= htmlspecialchars($tablet['existing_user']) ?></td>
                    <td><?= htmlspecialchars($tablet['status']) ?></td>
                    <td><?= htmlspecialchars($tablet['purchase_date']) ?></td>
                    <td><?= htmlspecialchars($tablet['warranty']) ?></td>
                    <td><?= htmlspecialchars($tablet['other_details']) ?></td>
                </tr>
            <?php endforeach; ?>
        </tbody>
    </table>
