...
$total_stmt = $pdo->query("SELECT COUNT(*) FROM network_equipment");
$total_rows = $total_stmt->fetchColumn();
$total_pages = ceil($total_rows / $limit);

// Fetch paginated network_equipment from the database
$stmt = $pdo->prepare("SELECT * FROM network_equipment ORDER BY device_name ASC LIMIT :start, :limit");
...
<html>
<head>
    <title>Network Equipments Inventory</title>
    ...
    <script>
        ...
        function toggleColumn(columnIndex) {
            const table = document.getElementById('network_equipmentTable');
        ...
        function searchTable() {
            ...
            const rows = document.querySelectorAll('#network_equipmentTable tbody tr');
            rows.forEach(row => {
...
<body>
    <h1>Network Equipments Inventory</h1>
    ...
    <table id="network_equipmentTable">
        <thead>
            <tr>
        ...
        <tbody>
            <?php foreach ($network_equipment as $computer): ?>
                <tr>
